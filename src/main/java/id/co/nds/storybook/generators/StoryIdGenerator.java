...
public class StoryIdGenerator implements IdentifierGenerator {
    ...
    public Serializable generate(SharedSessionContractImplementor ssci, Object o) throws HibernateException {
        ...
        try {
            PreparedStatement ps = connection.prepareStatement("SELECT COUNT(*) AS seq FROM ms_story");

            ResultSet rs = ps.executeQuery();
            if(rs.next()) {
                int seq = rs.getInt("seq") + 1;
                String code = String.format("story%05d", seq);
                System.out.println("Generated Story Code: " + code);
                return code;
            }
            else {
